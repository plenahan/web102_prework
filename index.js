...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++){
        ...
        const display = `<h3>${games[i]["name"]}</h3><p>${games[i]["description"]}</p><img class=\"game-img\" src=\"${games[i]["img"]}\" ></img>`;
        div.innerHTML = display;
        div.addEventListener("mouseover", function(){ toggleView(div, games[i], false)});
        div.addEventListener("mouseout", function(){ toggleView(div, games[i], true)});
...
function toggleView(div, game, on) {
    ...
    pledged.innerHTML = `Pledged: ${game["pledged"]}`;
    backers.innerHTML = `Backers: ${game["backers"]}`;
    if(on){
        ...
        div.appendChild(pledged);
        div.appendChild(backers);
    }
}
...
Currently, ${unfunded} ${unfunded == 1 ? "game remains" : "games remain"} unfunded. 
We need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let des = document.createElement("p");
des.innerHTML = displayStr;
...
let [game1, game2] = [sortedGames[0], sortedGames[1]];

// create a new element to hold the name of the top pledge game, then append it to the correct element
let game1Elem = document.createElement("h3");
game1Elem.innerHTML = game1["name"];
firstGameContainer.appendChild(game1Elem);
// do the same for the runner up item
let game2Elem = document.createElement("h3");
